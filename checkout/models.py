...
class Order(models.Model):
    ...
    def total_update(self):
        """
        Update total as a line item is added,
        ...
        self.overal_total = self.order_total + self.delivery_cost
        self.save()
...
class OrderLineTotal(models.Model):
    order = models.ForeignKey(Order, null=False, blank=False, on_delete=models.CASCADE, related_name='itemlines')
    book = models.ForeignKey(Book, null=False, blank=False, on_delete=models.CASCADE)
    quantity = models.IntegerField(null=False, blank=False, default=0)
    itemline_total = models.DecimalField(max_digits=6, decimal_places=2, null=False, blank=False, editable=False)
